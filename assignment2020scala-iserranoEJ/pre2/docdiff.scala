...
def occurrences(xs: List[String]): Map[String, Int] = xs.groupBy(l => l).map(n => (n._1, n._2.length))
...
def prod(lst1: List[String], lst2: List[String]) : Int = (for((k,v) <- occurrences(lst1) if(occurrences(lst2).contains(k))) yield v * occurrences(lst2)(k)).sum
...
def overlap(lst1: List[String], lst2: List[String]) : Double = (prod(lst1,lst2)).toDouble / (max(prod(lst1,lst1), prod(lst2,lst2)))
