...
def count_tours(dim: Int, path: Path) : Int = {
  ...
  else(for(n <- legal_moves(dim, path, path.head)) yield count_tours(dim, n::path)).sum
}

//time_needed(count_tours(5, List((0,0))).length)

def enum_tours(dim: Int, path: Path) : List[Path] = {
  if(path.length == dim*dim) List(path)
  else (for(n <- legal_moves(dim, path, path.head)) yield enum_tours(dim, n::path)).flatten
}

// enum_tours(5, List((0,0))).length
// time_needed(enum_tours(5, List((0,0))).length)
...
def first_tour(dim: Int, path: Path) : Option[Path] = path match {
  ...
  case y => first(legal_moves(dim, y, y(0)), x => first_tour(dim, x +: y))
}
